...
describe('getAllConfigFilesInDirectory', () => {
  it('returns default and local files', () => {
    ...
    expect(files).toEqual(['/test/default.yaml', '/test/local.yaml']);
  });
  it('returns default, environment, and local files', () => {
    const files = getAllConfigFilesInDirectory('/test', 'test');
    expect(files).toEqual(['/test/default.yaml', '/test/test.yaml', '/test/local.yaml']);
  });
});
...
describe('getAllConfigLocations', () => {
  ...
  it('adds an extra config file', () => {
    const files = getAllConfigLocations('/test', '/test', 'test', '/test/test-extra.yaml');
    expect(files).toEqual(['/test/default.yaml', '/test/test.yaml', '/test/local.yaml', '/test/test-extra.yaml']);
  });
  it('does not add extra config file when it is the same', () => {
    const files = getAllConfigLocations('/test', '/test', 'test', '/test/test.yaml');
    expect(files).toEqual(['/test/default.yaml', '/test/test.yaml', '/test/local.yaml']);
  });
});
...
describe('readConfig', () => {
  ...
  describe('errors', () => {
    ...
    it('fails on errors different than ENOENT', () => {
      expect(() => readConfig(process.env, undefined, undefined, dir)).toThrowError(/EISDIR/);
    });
  });
