...
describe('server/app', () => {
  ...
  describe('POST /call', () => {
    ...
    it('redirects to specific call', () => {
      return request(app).post('/call').send('call=test%20id').expect(302).expect('location', `${BASE_URL}/call/test%20id`);
    });
  });

  describe('GET /call/<uuid>', () => {
    it('renders call page', () => {
      return request(app).get(`${BASE_URL}/call/test`).expect(200);
    });
