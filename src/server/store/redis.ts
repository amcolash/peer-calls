...
export class RedisStore implements Store {
  constructor(protected readonly redis: RedisClient, protected readonly prefix: string) {}
  ...
  async getMany(keys: string[]): Promise<Array<string | undefined>> {
    const result = await Promise.all(keys.map((key) => this.redis.get(this.getKey(key))));
