  ...
  describe(MEDIA_STREAM, () => {
    ...
    describe('using navigator.mediaDevices.getUserMedia', () => {
      ...
      describe('reducers/peers', () => {
        ...
        it('adds local camera stream to all peers', async () => {
          await dispatch();
          peers.forEach((peer) => {
            expect((peer.addTrack as jest.Mock).mock.calls).toEqual([[track, stream]]);
            expect((peer.removeTrack as any).mock.calls).toEqual([]);
          });
          await dispatch();
          peers.forEach((peer) => {
            expect((peer.addTrack as jest.Mock).mock.calls).toEqual([
              [track, stream],
              [track, stream],
            ]);
            expect((peer.removeTrack as jest.Mock).mock.calls).toEqual([[track, stream]]);
          });
        });
      });
    });
    ...
    ['getUserMedia', 'mozGetUserMedia', 'webkitGetUserMedia'].forEach((item) => {
      describe('compatibility: navigator.' + item, () => {
        beforeEach(() => {
          const getUserMedia: typeof navigator.getUserMedia = (constraint, onSuccess, onError) => onSuccess(stream);
