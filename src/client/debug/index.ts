...
async function checkTURNServer(turnConfig: ICEServer, timeoutDuration = 5000) {
  ...
  async function start() {
    const PeerConnection =
      ...
      ((window as unknown) as { mozRTCPeerConnection: RTCPeerConnection }).mozRTCPeerConnection ||
...
checkTURNServer(config.iceServers[0], 10000).then(console.log.bind(console)).catch(console.error.bind(console));
