...
describe('components/Input', () => {
  ...
  async function render() {
    ...
    await new Promise<Input>((resolve) => {
      ReactDOM.render(<Input ref={(input) => resolve(input!)} sendMessage={sendMessage} />, div);
    });
    node = div.children[0];
  }
  ...
  describe('send message', () => {
    ...
    describe('handleSubmit', () => {
      ...
      it('sends a message', () => {
        ...
        expect(sendMessage.mock.calls).toEqual([[{ payload: message, type: 'text' }]]);
      });
      ...
      it('sends a nickname command', () => {
        ...
        TestUtils.Simulate.submit(node);
        expect(sendMessage.mock.calls).toEqual([[{ payload: { nickname: 'john' }, type: 'nickname' }]]);
      });
      ...
      it('does not fail when command is empty', () => {
        ...
        TestUtils.Simulate.submit(node);
        expect(sendMessage.mock.calls).toEqual([[{ payload: { nickname: '' }, type: 'nickname' }]]);
      });
      ...
      it('sends message when command is invalid', () => {
        ...
        TestUtils.Simulate.submit(node);
        expect(sendMessage.mock.calls).toEqual([[{ payload: '/nick', type: 'text' }]]);
      });
    });
